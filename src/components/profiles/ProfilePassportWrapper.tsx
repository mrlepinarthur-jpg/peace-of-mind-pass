...
import {
  ...
  Scale, KeyRound, HeartHandshake, PawPrint,
} from "lucide-react";
...
interface ProfilePassportViewProps {
  profileId: string;
  onBack: () => void;
}

const ProfilePassportView = ({ profileId, onBack }: ProfilePassportViewProps) => {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (!user) return;
    setOwnerName((user.user_metadata?.full_name as string) || user.email?.split("@")[0] || "vous");
  }, [user]);
  ...
  return (
    <div className="pb-24">
      <Button variant="ghost" size="sm" onClick={onBack} className="mb-3 gap-2">
...
export default ProfilePassportView;
